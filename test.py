...
from dataset import TestDataset_registration
...
def test(args):
    ...
    state_dict = torch.load('checkpoint_segmif/ir_encoder.pkl', map_location = 'cpu')
    ir_encoder.load_state_dict(state_dict, strict=False)    
    state_dict = torch.load('checkpoint_segmif/vis_encoder.pkl', map_location = 'cpu')
    vis_encoder.load_state_dict(state_dict, strict=False)
    state_dict = torch.load('checkpoint_segmif/fus_decoder.pkl', map_location='cpu')
    fus_decoder.load_state_dict(state_dict, strict=False)
    state_dict = torch.load('checkpoint_segmif/regis.pkl', map_location='cpu')
    net.load_state_dict(state_dict)
    save_dir = "results/segmif"
    ...
    test_data_path ='../registration_Segmif_bigfield/test'
    ...
    os.makedirs(os.path.join(save_dir,'registration'), exist_ok=True)  
    os.makedirs(os.path.join(save_dir,'fusion'), exist_ok=True)  
