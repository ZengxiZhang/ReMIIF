...
class DAM(nn.Module):
    ...
    def __init__(self, in_planes, n_head, d_k, d_v):
        ...
        self.patch_unembed = PatchUnEmbed(embed_dim=in_planes)
        self.slf_att = ESSAttn(in_planes)
    ...
    def forward(self, input1, input2, kernel, pad, show = False):
        ...
        # else:
        x_size = (input1.shape[2], input1.shape[3])
        slf_1 = self.patch_unembed(self.slf_att(self.norm(self.patch_embed(input1))), x_size)
        slf_2 = self.patch_unembed(self.slf_att(self.norm(self.patch_embed(input2))), x_size)
        # print(slf_1.shape)
